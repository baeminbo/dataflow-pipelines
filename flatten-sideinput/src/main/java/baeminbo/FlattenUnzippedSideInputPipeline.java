...
public class FlattenUnzippedSideInputPipeline {

  private static final Logger LOG = LoggerFactory.getLogger(FlattenUnzippedSideInputPipeline.class);
  ...
  public static void main(String[] args) {
    ...
    PCollection<String> left = main.apply("Left", ParDo.of(new DoFn<String, String>() {
      ...
      public void processElement(ProcessContext context, BoundedWindow window) {
        ...
        LOG.info("Left; {} -> {}, timestamp:{}, window:{}", element, outputElement, timestamp,
            window);
        context.output(outputElement);
      }
    }));

    PCollection<String> right = main.apply("Right", ParDo.of(new DoFn<String, String>() {
      @ProcessElement
      public void processElement(ProcessContext context, BoundedWindow window) {
        String element = context.element();
        Instant timestamp = context.timestamp();
        String outputElement = "Right-" + element;
        LOG.info("Right; {} -> {}, timestamp:{}, window:{}", element, outputElement, timestamp,
            window);
        context.output(outputElement);
      }
    }));
